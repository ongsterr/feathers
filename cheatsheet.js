...
service.publish((data, hook) => {}) // registers a publishing function for a specific service for a specific event or all events if no event name was given
app.publish((data, hook) => {}) // registers an event publishing callback

app.on('connection', (connection) => {}) // fired every time a new real-time connection is established
app.on('disconnection', (connection) => {})
app.on(
	'login',
	(authenticationResult, params, context) => {}
...
app.configure(socketio(options[(_, callback)])) // sets up the Socket.io transport with the given Socket.io options object and optionally calls the callback
...
authService.create({
	// Authenticate and create a JWT using the local strategy email/password
	strategy: 'local',
...
const options = {
	authentication: {
		oauth: {
			...
			github: {
				// The per-strategy configuration
				key: '<Client ID>',
